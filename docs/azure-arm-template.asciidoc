...
the provided vNetClusterSubnetName. Defaults to `10.126.181.4`.

When deploying a new virtual network, the following parameters also apply

`vNetNewAddressPrefix`::
The address space when creating a new virtual network. Defaults to `10.126.181.0/24`.

`vNetNewClusterSubnetAddressPrefix`::
The address space of the `vNetClusterSubnetName` subnet. Defaults to `10.126.181.0/25`.
